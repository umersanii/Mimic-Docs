...
2. **Collapsed to 1 actuated joint per finger**, matching the real hardware's 1-servo-per-finger
   ...
   `<mimic>` it 1:1. The pinky/ring-finger abduction joints were converted to `type="fixed"`, so no
...
closely together, and at that scale, the default shadow map produces shadow acne (flickering
